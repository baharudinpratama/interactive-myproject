...
export default function IconPicker({ icon, color, onSelected }: { icon: string; color: string; onSelected: (selectedIcon: string, selectedColor: string) => void; }) {
  ...
  const [selectedColor, setSelectedColor] = useState(color === "" ? "#B2BBC6" : color);
  const [iconSearchInput, setIconSearchInput] = useState("");
  const [selectedIcon, setSelectedIcon] = useState(icon === "" ? "solar:alt-arrow-down-outline" : icon);
  ...
  const handleColorInput = (input: React.ChangeEvent<HTMLInputElement> | string) => {
    ...
    onSelected(selectedIcon, newColor);
  };
  ...
  const handleIconSelect = (icon: string) => {
    ...
    onSelected(icon, selectedColor);
  }

  return (
    <div className="absolute top-[115px] left-[80px] w-[200px] p-[15px] rounded-[8px] border border-white-active bg-white shadow-md z-[100]" onClick={(e) => e.stopPropagation()}>
      <div className="flex flex-col gap-[13px]">
            ...
            {colors.map((color, index) => {
              return (
                <MyButton
                  ...
                  radius="full"
                  value={color}
                  onPress={() => handleColorInput(color)}
        ...
        <div className="flex flex-col gap-[13px]">
          <Input
            ...
            startContent={
              <Icon icon="solar:magnifer-linear" height={16} style={{ color: "var(--grey-lighter)" }} />
            }
            placeholder={
              "Search"
            }
          ...
          <div className="grid grid-cols-6 gap-y-[13px]">
            {icons.map((sample: any, index: any) => {
              return (
                <button key={index} className="m-0 p-0" onClick={() => handleIconSelect(sample)}>
                  <Icon icon={sample} width={22} style={{ color: "#B2BBC6" }} />
                </button>
              );
            })}
          </div>
